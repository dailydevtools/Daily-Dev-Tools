...
export default function ToolMarquee() {
    ...
    // Select a subset of tools for the marquee to improve performance (reduce DOM nodes)
    const marqueeTools = tools.slice(0, 20);
    // Duplicate for seamless loop
    const allTools = [...marqueeTools, ...marqueeTools];
    ...
    return (
        ...
        >
            <div
                className="flex gap-3 w-fit animate-marquee group-hover:[animation-play-state:paused] group-focus-within:[animation-play-state:paused]"
            >
                {allTools.map((tool, index) => (
                    <Link
                        key={`${tool.id}-${index}`}
                        href={`/tools/${tool.id}`}
                        className="flex items-center gap-2 px-4 py-2.5 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-full no-underline whitespace-nowrap transition-all duration-200 flex-shrink-0 hover:bg-[var(--card-hover-bg)] hover:border-[#fb923c] hover:scale-105 focus:scale-105 focus:border-[#fb923c] outline-none"
                        aria-label={tTools(`${tool.id}.name`, { fallback: tool.name })}
                    >
