...
export default function LiquidTabs({
    ...
    return (
        <div className="flex justify-center mb-10 text-center w-full">
                ...
                {tabs.map((tab, i) => (
                    <div key={tab} className="flex items-center">
                        <button
                            ...
                            className={`relative px-8 py-2.5 text-sm font-medium rounded-full transition-colors duration-200 z-10 outline-none select-none
                            ${activeTab === tab ? 'text-[var(--title-color)]' : 'text-[var(--muted-text)] hover:text-[var(--title-color)]'}`}
                        >
                            {activeTab === tab && (
                                <motion.div
                                    layoutId="active-tab-pill" // Ensure this is unique per page if multiple instances exist, or shared if intended
