...
import { EditorService, OutputType } from '../services/editor-service';

export class DailyDevToolsPanel {
    public static currentPanel: DailyDevToolsPanel | undefined;
        ...
        this._panel.webview.onDidReceiveMessage(
            async (message: any) => {
                switch (message.command) {
                    case 'executeTool':
                        const result = await ToolExecutionService.getInstance().executeTool(message.toolId, { text: message.text });
                        this._panel.webview.postMessage({ command: 'toolResult', result });
                        break;
                    case 'applyOutput':
                        await EditorService.applyOutput(message.text, message.outputType as OutputType);
                        break;
                }
            },
            null,
